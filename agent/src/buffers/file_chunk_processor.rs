use super::{hash_file, temp_folder_path};
...
#[derive(Debug)]
pub struct FileChunkProcessor {
    id: String,
    path: PathBuf,
    transfer_states: Arc<Mutex<HashMap<u64, TransferState>>>,
}

impl FileChunkProcessor {
    ...
    fn lock_transfer_states(&self) -> Result<MutexGuard<'_, HashMap<u64, TransferState>>> {
    ...
    fn temp_path_ref(&self, id: u64) -> PathBuf {
        temp_folder_path(&self.id).join(format!("{id}.tmp"))
    }

    pub fn process_chunked_transfer(&self, op: ChunkedTransferOp) -> Result<()> {
        match op {
            ...
            ChunkedTransferOp::End {
                ...
                hash,
                metadata,
            } => self.handle_end(id, &path.resolve(&self.path), hash, &metadata),
...
mod tests {
    use super::*;
    use crate::models::{FileType, Permissions, RelativePath};
    use crate::protocol::ChunkedTransferOp;
    use tempfile::tempdir;

    fn create_test_folder() -> (FileChunkProcessor, tempfile::TempDir) {
        let temp_dir = tempdir().unwrap();
        let folder = FileChunkProcessor::new(
